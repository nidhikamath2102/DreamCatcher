...
private const val DREAM_LAST_UPDATED_PATTERN = "Last updated 2024-02-04 at ..:[03]4:24 [AP]M"
...
class P2BTest {
    ...
    fun dreamFourCheckDetails() {
        ...
        onView(withId(R.id.last_updated_text))
            .check(matches(withPattern(DREAM_LAST_UPDATED_PATTERN)))
    ...
    fun dreamFourRotateClickDeferThenBackRotateAndConfirmList() {
        ...
        checkEntryGone(2)
        onView(withId(R.id.last_updated_text))
            .check(matches(not(withPattern(DREAM_LAST_UPDATED_PATTERN))))
        pressBack()
        scenario.recreate()
        onView(withId(R.id.dream_recycler_view))
            .check(
    ...
    fun dreamEighteenAddTwoReflectionsCheckLastUpdatedRotateBackCheckList() {
        ...
        onView(withId(R.id.last_updated_text))
            .check(matches(not(withPattern(DREAM_LAST_UPDATED_PATTERN))))
        pressBack()
        onView(withId(R.id.dream_recycler_view)).perform(scrollToPosition<DreamHolder>(0))
        onView(withId(R.id.dream_recycler_view))
            .check(
                matches(
                    atPosition(
